...
func SetRTBStore(ctx context.Context, schema *types.Schema, mgmt *config.ScaledContext) {
	...
	t := &transform.Store{
		...
		Transformer: func(apiContext *types.APIContext, schema *types.Schema, data map[string]interface{}, opt *types.QueryOptions) (map[string]interface{}, error) {
			if id, ok := data[client.BusinessRoleTemplateBindingFieldUserId].(string); ok && id != "" {
...
func (s *Store) Create(apiContext *types.APIContext, schema *types.Schema, data map[string]interface{}) (map[string]interface{}, error) {
	if principalID, ok := data[client.BusinessRoleTemplateBindingFieldUserPrincipalId].(string); ok && principalID != "" && !strings.HasPrefix(principalID, "local://") {
